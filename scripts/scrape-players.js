...
import { PLAYERS_TO_SCRAPE } from './players-list.js';
...
async function fetchWikipedia(wikiSlug) {
 ...
 */
async function fetchImageUrl(wikiSlug) {
...
function detectProStartDecade(introText, fields, birthYear) {
        ...
        if (val) {
            ...
            if (match) {
                ...
                if (year >= 1990 && year < 2030)
                    return yearToDecade(year);
            }
        }
    }
    const proMatch = introText.match(/turned\s+(?:professional|pro)\s+in\s+(\d{4})/i);
    if (proMatch)
        return yearToDecade(parseInt(proMatch[1], 10));
    if (birthYear)
        return yearToDecade(birthYear + 18);
    return '2010s';
}
...
function detectGender(_wikiSlug, introText, categories, slug) {
    ...
    if (femaleSignals.some(s => combined.includes(s)))
        return 'F';
    const maleSignals = ["men's", ' he ', ' his badminton', 'male player'];
    if (maleSignals.some(s => combined.includes(s)))
        return 'H';
    const femaleSlugs = [
    ...
    if (femaleSlugs.includes(slug))
        return 'F';
    return 'H';
}
async function scrapePlayer(entry) {
    const errors = [];
    const warnings = [];
    try {
        ...
        if (!birthDate)
            warnings.push('birth date missing or unparseable');
        if (age !== null && (age < 15 || age > 80))
            warnings.push(`suspicious age: ${age}`);
        // Country
        ...
        if (!continent)
            warnings.push(`unknown continent for country: "${countryEn}"`);
        if (!countryCode)
            warnings.push(`unknown country code for: "${countryEn}"`);
        // Height
        const heightRaw = getField(fields, ...FIELD_LABELS.height);
        const heightCm = heightRaw ? parseHeight(heightRaw) : null;
        if (!heightCm)
            warnings.push('height missing or unparseable');
        // Handedness
        ...
        if (handRaw) {
            ...
            if (parsed)
                ...
                warnings.push(`unparseable handedness: "${handRaw}"`);
        }
        else {
        ...
        if (!rankingNum)
            warnings.push('best ranking missing');
        ...
        const ageBracket = age !== null ? ageToBracket(age) : '30-35';
        const heightBracket = heightCm !== null ? heightToBracket(heightCm) : '175-180';
        const bestRanking = rankingNum !== null ? rankingToTier(rankingNum) : 'Top 20';
        const majorTitles = titlesToTier(titlesCount);
        const bestOlympicMedal = olympicMedal;
        ...
        if (!isDryRun) {
            ...
            if (sourceUrl) {
                ...
                if (!imageUrl)
                    warnings.push('image download failed, imageUrl set to null');
            }
            else {
                imageUrl = null;
        ...
        return { player: validated.data, slug: entry.slug, errors, warnings };
    }
    catch (err) {
...
async function main() {
    ...
    if (suspectPlayers.length === 0) {
        ...
        suspectPlayers.forEach(p => {
            const nullFields = [];
            if (!p.imageUrl)
                nullFields.push('imageUrl');
            if (p.countryCode === 'XX')
                nullFields.push('countryCode');
            if (p.country === 'Inconnu')
                nullFields.push('country');
