...
import type { OlympicMedal } from '../src/types/player.js';

/**
 * Extracts the best Olympic medal from the Wikipedia page.
 * Checks: Medal record table, categories, intro text.
 */
export function extractOlympicMedal($: CheerioAPI, introText: string, categories: string[]): OlympicMedal {
  // Check medal record table for Olympic rows
  let bestMedal: OlympicMedal = 'Aucune';

  // Look for medal record section
  $('table').each((_i, table) => {
    const tableText = $(table).text().toLowerCase();
    if (!tableText.includes('olympic')) return;

    $(table).find('tr').each((_j, row) => {
      const rowText = $(row).text().toLowerCase();
      if (!rowText.includes('olympic')) return;
  ...
  if (bestMedal !== 'Aucune') return bestMedal;

  // Check categories
  const catString = categories.join(' ');
  if (catString.includes('olympic gold')) return 'Or';
  if (catString.includes('olympic silver')) return 'Argent';
  if (catString.includes('olympic bronze')) return 'Bronze';

  // Check intro text
  const intro = introText.toLowerCase();
  if (intro.includes('olympic gold') || intro.includes('olympic champion')) return 'Or';
  if (intro.includes('olympic silver')) return 'Argent';
  if (intro.includes('olympic bronze')) return 'Bronze';
