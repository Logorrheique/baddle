import type { Hand } from '../src/types/player.js';
 ...
 */
export function parseHandedness(value: string): Hand | null {
  if (!value) return null;
  const lower = value.toLowerCase();
  if (lower.includes('left')) return 'Gaucher';
  if (lower.includes('right')) return 'Droitier';
  return null;
}
 ...
 */
export function parseHighestRanking(value: string): number | null {
  if (!value) return null;
