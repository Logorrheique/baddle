import type { Discipline } from '../src/types/player.js';
 ...
 */
export function detectDiscipline(introText: string, categories: string[]): Discipline {
  ...
  if (hasMixed && !hasSingles) return 'Double mixte';
  if (hasDoubles && !hasSingles && !hasMixed) return 'Double';
  ...
  if (hasMixed) return 'Double mixte';
  if (hasDoubles) return 'Double';
  return 'Simple';
}
 ...
 */
export function detectRetired(introText: string, categories: string[]): boolean {
