...
const CONTINENT_EN: Record<string, string> = {
  'Asie': 'Asia', 'Europe': 'Europe', 'Amériques': 'Americas',
  'Océanie': 'Oceania', 'Afrique': 'Africa',
};
...
export function displayCellValue(
  attribute: 'gender' | 'continent' | 'status' | 'discipline' | 'hand' | 'ageBracket' | 'heightBracket' | 'bestRanking',
  rawValue: string,
  ...
  if (lang === 'en') {
    if (attribute === 'continent')  return CONTINENT_EN[rawValue]  ?? rawValue;
...
export function displayCountry(countryFr: string, lang: Language): string {
  if (lang === 'en') return COUNTRY_FR_TO_EN[countryFr] ?? countryFr;
  return countryFr;
}
