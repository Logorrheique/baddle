...
const BASE: Player = {
  ...
  gender: 'H', country: 'Chine', countryCode: 'CN',
...
describe('exact attributes', () => {
  ...
  it('country exact correct', () => expect(compareGuess(g({ country: 'Chine' }), BASE).country.state).toBe('correct'));
  it('country exact incorrect', () => expect(compareGuess(g({ country: 'France' }), BASE).country.state).toBe('incorrect'));
